...
function ApplicantPortfolio() {
  ...
  const [detailedPosting, setDetailedPosting] =
    useState<DetailedPostingApplication>();
  let { id } = useParams();
  ...
  const getData = async () => {
    ...
    try {
      ...
      toast({
        title: "Hata",
        description: "Başvuran detayları getirilirken bir hata oluştu.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <IsReadonlyProvider isReadonly={true}>
